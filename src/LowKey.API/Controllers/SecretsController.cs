...
public class SecretsController : ControllerBase
{
    ...
    public SecretsController(ILogger<SecretsController> logger)
    {
        ...
        var keyoptions = new KeyClientOptions(KeyClientOptions.ServiceVersion.V7_4)
        {
            DisableChallengeResourceVerification = true
        };
        _secretClient = new SecretClient(new Uri("https://localhost:8443/"),  new NoopCredentials(), GetClientOptions(options));
        _keyClient = new KeyClient(new Uri("https://localhost:8443/"),  new NoopCredentials(), GetClientOptions(keyoptions));
    }
    ...
    [HttpPost]
    [Route("GetRSAKey")]
    public IActionResult PostKey(string name)
    {
        var secret = _keyClient.GetKey(name);
